...
import { patchMenuButtons, detectDeviceFromProgram, detectSSD1309Patch } from '../operations/patch.js';
...
export class FxParsedSlot {
   ...
   */
  /** @type {string|null} Cached detected device type */
  _detectedDevice = undefined;

  /** @type {boolean|undefined} Cached SSD1309 patch detection */
  _hasSSD1309Patch = undefined;

  /** @type {Array|null} Available binaries from source .arduboy package */
  arduboyBinaries = null;

  /** @type {number} Index of the currently active binary in arduboyBinaries (-1 = none/N/A) */
  activeBinaryIndex = -1;
  ...
  /**
   * Detected target device from program binary analysis.
   * Cached lazily — call invalidateDetectedDevice() when programRaw changes.
   * @returns {string|null} DEVICE_TYPE value, or null if no program
   */
  get detectedDevice() {
    if (this._detectedDevice === undefined) {
      this._detectedDevice = detectDeviceFromProgram(this.programRaw);
    }
    return this._detectedDevice;
  }

  /** Invalidate cached device detection (call when programRaw changes). */
  invalidateDetectedDevice() {
    this._detectedDevice = undefined;
    this._hasSSD1309Patch = undefined;
  }

  /**
   * Whether the program binary has the SSD1309 display patch applied.
   * Cached lazily — invalidated alongside device detection.
   * @returns {boolean}
   */
  get hasSSD1309Patch() {
    if (this._hasSSD1309Patch === undefined) {
      this._hasSSD1309Patch = detectSSD1309Patch(this.programRaw);
    }
    return this._hasSSD1309Patch;
  }
}
...
async function compileSingleSlot(slot, currentPage, previousPage) {
  ...
  // Apply menu button ISR patch (hold UP+DOWN for 2s → bootloader menu)
  if (program.length > 0) {
...
export function scanFxCartHeaders(fullData) {
  ...
  return {
    count: slots.length,
    categories: Math.max(0, categories - 1),
    games,
