...
import { detectDeviceFromProgram } from '../operations/patch.js';
...
export async function readArduboyFile(fileData, filename = 'unknown.arduboy') {
  ...
  for (const bin of binaryList) {
    ...
    // Resolve device: prefer metadata, fall back to auto-detection from hex
    let device = bin.device || null;
    if (!device && hexRaw) {
      try {
        const parsed = parseIntelHex(hexRaw);
        device = detectDeviceFromProgram(parsed.data) || 'Arduboy';
      } catch {
        device = 'Arduboy';
      }
    }
