...
export async function buildFxData(project, entryFile, options = {}) {
  ...
  // Helper: resolve a file path, falling back to basename match in the VFS
  // (the VFS may store files flat while fxdata.txt uses relative paths like ../assets/foo.png)
  function resolveWithFallback(relativePath, fromFile) {
    const resolved = project.resolvePath(relativePath, fromFile);
    if (project.hasFile(resolved)) return resolved;
    const basename = resolved.split('/').pop();
    const match = project.listFiles().find((f) => f.split('/').pop() === basename);
  ...
  const callbacks = {
    ...
    resolveInclude(includePath, fromFile) {
      const resolved = resolveWithFallback(includePath, fromFile);
    ...
    async resolveImage(imagePath, fromFile, opts) {
      const resolved = resolveWithFallback(imagePath, fromFile);
    ...
    resolveRaw(rawPath, fromFile) {
      const resolved = resolveWithFallback(rawPath, fromFile);
...
function generateHeader(bodyLines, fxDataPage, dataSize, fxSavePage, saveSize, toolVersion) {
  ...
  lines.push('// Example: FX::begin(FX_DATA_PAGE);');
  lines.push('');
  lines.push('// Use FX::display(CLEAR_BUFFER); instead of arduboy.display(); and arduboy.clear();');
  lines.push('// Example: FX::display(CLEAR_BUFFER);');
  lines.push('');
