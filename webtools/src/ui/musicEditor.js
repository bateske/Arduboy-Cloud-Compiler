...
export class MusicEditor {
  ...
  _grabRefs() {
    ...
    this._btnPushToCode = $('#btn-mus-push-code');
    this._btnPushSketch = $('#btn-mus-push-sketch');
  ...
  _updateUsageDisplay() {
    ...
    if (this._usageOutput) {
      ...
      if (showFull) applyExampleLinks(this._usageOutput);
    }
    // Toggle push-sketch button visibility
    if (showFull) {
  ...
  async _pushToCode() {
    ...
    // Search for existing file across all tabs
    let tab = api.findTabByBasename(filename);

    if (tab) {
      // File exists — check if the array variable already present
      const existing = tab.model.getValue();
      const escaped = arrayName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const varPattern = new RegExp('\\b' + escaped + '\\b');

      if (varPattern.test(existing)) {
        const replace = await showConfirm(
          `"${arrayName}" already exists in ${tab.filename}. Replace it?`,
          { title: 'Replace Variable', okLabel: 'Replace', cancelLabel: 'Cancel' },
        );
        if (!replace) return;

        // Replace the PROGMEM block
        const newContent = this._replaceVariableBlock(existing, arrayName, code);
        tab.model.setValue(newContent);
        showToast(`Replaced "${arrayName}" in ${tab.filename}`, 'success');
      } else {
        // Append
        const sep = existing.endsWith('\n') ? '\n' : '\n\n';
        tab.model.setValue(existing + sep + code + '\n');
        showToast(`Added "${arrayName}" to ${tab.filename}`, 'success');
      }

      api.switchToTab(tab.id);
    } else {
      // Create new file
      tab = api.createTab(filename, code + '\n');
      api.sortTabs();

      // Add #include to the .ino file
      const inoTab = api.findInoTab();
      if (inoTab) {
        const inoSrc = inoTab.model.getValue();
        const includePattern = new RegExp('#\\s*include\\s+["\']' + filename.replace('.', '\\.') + '["\']');
        if (!includePattern.test(inoSrc)) {
          ...
          if (lastInclude >= 0) {
            lines.splice(lastInclude + 1, 0, `#include "${filename}"`);
          } else {
            lines.unshift(`#include "${filename}"`);
          }
          inoTab.model.setValue(lines.join('\n'));
        }
      }

      showToast(`Created ${filename} and added include to sketch`, 'success');
      api.switchToTab(tab.id);
    }
  ...
  async _pushSketchToCode() {
    ...
    const arrayName = this._arrayNameInput?.value?.trim() || 'song';
    const filename = arrayName.charAt(0).toUpperCase() + arrayName.slice(1) + '.ino';
  ...
  /**
   * Replace a variable's PROGMEM block in existing source code.
   */
  _replaceVariableBlock(source, varName, newCode) {
    const escaped = varName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Match PROGMEM array declarations (uint16_t for tones, byte for playtune)
    const progmemRe = new RegExp(
      '^[ \\t]*const\\s+(?:uint16_t|byte)\\s+' + escaped + '\\s*\\[\\][\\s\\S]*?^[ \\t]*\\};',
      'gm',
    );
    ...
    // Replace first block, remove subsequent blocks
    let result = '';
    let cursor = 0;
    for (let i = 0; i < merged.length; i++) {
      result += source.slice(cursor, merged[i][0]);
      if (i === 0) result += newCode;
      cursor = merged[i][1];
    }
