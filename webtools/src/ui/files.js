...
/**
 * Trigger a file save dialog for binary data, falling back to a direct
 * browser download when the File System Access API is unavailable.
...
export async function downloadBlob(data, filename, mimeType = 'application/octet-stream') {
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      if (ext) {
        opts.types = [{ description: filename, accept: { [mimeType]: [ext] } }];
      }
