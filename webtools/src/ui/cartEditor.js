...
import {
  ...
  patchSSD1309, unpatchSSD1309, patchCSPin,
...
const STORAGE_KEY = 'cartEditor-state';
const STORAGE_SAVE_DELAY = 500; // ms debounce for localStorage writes

/** Short display labels for device types */
const DEVICE_LABELS = {
  [DEVICE_TYPE.ARDUBOY_FX]: 'FX',
  [DEVICE_TYPE.ARDUBOY_FX_C]: 'FX-C',
  [DEVICE_TYPE.ARDUBOY_MINI]: 'Mini',
};
...
export class CartEditor {
  ...
  /** @type {string} Cart-level target device (ephemeral, not stored in .bin) */
  targetDevice = DEVICE_TYPE.ARDUBOY_FX_C;

  /** @type {string} Display type for patching (SSD1306 or SSD1309) */
  displayType = 'SSD1306';
  ...
  _bindToolbar() {
    ...
    // Cart target device selector
    const targetSel = document.getElementById('cart-target-device');
    if (targetSel) {
      targetSel.addEventListener('change', () => {
        this.targetDevice = targetSel.value;
        this._scheduleSave();
        this.render();
      });
    }

    // Display type selector (SSD1306 / SSD1309)
    const displaySel = document.getElementById('cart-display-type');
    if (displaySel) {
      displaySel.addEventListener('change', () => {
        this.displayType = displaySel.value;
        this._scheduleSave();
      });
    }
    ...
    panel.addEventListener('drop', async (e) => {
      ...
      if (name.endsWith('.arduboy')) {
        ...
        try {
          ...
          slot.arduboyBinaries = newSlot.arduboyBinaries;
          slot.activeBinaryIndex = newSlot.activeBinaryIndex;
          slot.invalidateDetectedDevice();
          this._markDirty();
          this.render();
  ...
  async newCart() {
    if ((this.dirty || this._hasNonTrivialContent()) && !await showConfirm('Current cart data will be lost. Create a new cart?')) return;
  ...
  async openBinFile(file) {
    if ((this.dirty || this._hasNonTrivialContent()) && !await showConfirm('Current cart data will be lost. Open a different cart?')) return;
  ...
  async saveBinFile() {
    ...
    // Check for device mismatches and warn the user
    const mismatches = this._getDeviceMismatches();
    if (mismatches.length > 0) {
      const list = mismatches.map((m) =>
        `  Slot ${m.index + 1}: ${m.title} (${m.detected})`
      ).join('\n');
      const result = await showConfirm(
        `${mismatches.length} game(s) compiled for a different device than the cart target (${this.targetDevice}):\n\n${list}`,
        {
          title: 'Device Mismatch',
          buttons: [
            { label: 'Save Anyway', value: 'save', className: 'btn btn-primary' },
            { label: '⚠️ Experimental: Patch CS Pin', value: 'patch', className: 'btn btn-warning' },
      ...
      if (!result) return;
      if (result === 'patch') {
        this._patchMismatchedCSPins(mismatches);
      }
    }

    try {
      const binary = await compileFxCart(this.slots);
      this._applyDisplayPatch(binary);
  ...
  async addGameFromFile(file, insertAt) {
    try {
      ...
      this._insertGameSlot(slot, insertAt);
  ...
  /**
   * Add a game slot from a pre-parsed ArduboyPackage object.
   * @param {ArduboyPackage} pkg
   * @param {number} [insertAt]
   */
  async addGameFromPackage(pkg, insertAt) {
    try {
      const slot = await this._slotFromPackage(pkg);
      this._insertGameSlot(slot, insertAt);
      showToast(`Added: ${slot.meta.title || 'game'}`, 'success');
    } catch (err) {
      showToast(`Failed to add game: ${err.message}`, 'error');
      console.error(err);
    }
  ...
  /** @private Insert a game slot at the given position (or after selected). */
  _insertGameSlot(slot, insertAt) {
    ...
    let idx = (insertAt != null && insertAt >= 1) ? insertAt : (this.selectedIndex >= 1 ? this.selectedIndex + 1 : this.slots.length);
    if (idx < 1) idx = 1;
  ...
  async loadFromDevice() {
    ...
    if ((this.dirty || this._hasNonTrivialContent()) && !await showConfirm('Current cart data will be lost. Load from device?')) return;
  ...
  async writeToDevice() {
    ...
    // Check for device mismatches before writing
    const mismatches = this._getDeviceMismatches();
    if (mismatches.length > 0) {
      const list = mismatches.map((m) =>
        `  Slot ${m.index + 1}: ${m.title} (${m.detected})`
      ...
      const result = await showConfirm(
        `${mismatches.length} game(s) compiled for a different device than the cart target (${this.targetDevice}):\n\n${list}`,
        {
          title: 'Device Mismatch',
          buttons: [
            { label: 'Write Anyway', value: 'write', className: 'btn btn-primary' },
            { label: '⚠️ Experimental: Patch CS Pin', value: 'patch', className: 'btn btn-warning' },
            { label: 'Cancel', value: false, className: 'btn btn-secondary' },
          ],
        },
    ...
    try {
      ...
      // Apply display patch based on selected display type
      this._applyDisplayPatch(binary);
  ...
  async exportSelectedSlot() {
    ...
    try {
      ...
      // Determine device type from binary analysis (more accurate than fxEnabled flag)
      const device = slot.detectedDevice || (slot.fxEnabled ? 'ArduboyFX' : 'Arduboy');
  ...
  _updateControlsVisibility() {
    ...
    const settingsRow = document.getElementById('cart-toolbar-settings');
  ...
  _createSlotElement(slot, index) {
    ...
    if (isBootloaderSlot) {
      ...
      let deviceBadge = '';
      if (slot.fxEnabled) {
        const detected = slot.detectedDevice;
        const normDetected = detected === DEVICE_TYPE.ARDUBOY_FX_C ? DEVICE_TYPE.ARDUBOY_MINI : detected;
        const normTarget = this.targetDevice === DEVICE_TYPE.ARDUBOY_FX_C ? DEVICE_TYPE.ARDUBOY_MINI : this.targetDevice;
        const isMatch = !detected || normDetected === normTarget;
        // For Mini/FX-C (same CS pin), defer label to user's target selection only when compatible
        const label = (detected && normDetected === DEVICE_TYPE.ARDUBOY_MINI)
          ? (isMatch ? (DEVICE_LABELS[this.targetDevice] || 'FX-C') : 'FX-C')
          : (detected && DEVICE_LABELS[detected]) || DEVICE_LABELS[this.targetDevice] || 'FX';
        deviceBadge = `<span class="slot-badge ${isMatch ? 'slot-badge-device-match' : 'slot-badge-device-mismatch'}">${this._esc(label)}</span>`;
      }
      const ssd1309Badge = slot.hasSSD1309Patch ? '<span class="slot-badge slot-badge-1309">1309</span>' : '';
      info.innerHTML = `
        <div class="slot-title">${this._esc(slot.meta.title || 'Untitled')} ${deviceBadge}${ssd1309Badge}</div>
        <div class="slot-meta">
    ...
    panel.innerHTML = `
      ...
      <div class="cart-detail-image-section">
        ...
        <div class="cart-detail-image-actions">
          <button class="btn btn-sm btn-secondary" id="btn-detail-image" title="Import image"><svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 10V1m0 0L5 4m3-3 3 3M2 12v2h12v-2"/></svg></button>
          <input type="file" id="detail-image-file" accept="image/*" class="file-input">
          <button class="btn btn-sm btn-outline" id="btn-detail-save-image" title="Export as PNG"><svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 1v9m0 0L5 7m3 3 3-3M2 12v2h12v-2"/></svg></button>
          <span class="cart-detail-image-sep"></span>
          <button class="btn btn-sm btn-secondary" id="btn-detail-edit-image" title="Open pixel editor">Edit</button>
          <button class="btn btn-sm btn-danger" id="btn-detail-revert-image" title="Revert to original image" style="display:none">Revert</button>
  ...
  _renderGameDetail(panel, slot) {
    ...
    const normDetected = detected === DEVICE_TYPE.ARDUBOY_FX_C ? DEVICE_TYPE.ARDUBOY_MINI : detected;
    const normTarget = this.targetDevice === DEVICE_TYPE.ARDUBOY_FX_C ? DEVICE_TYPE.ARDUBOY_MINI : this.targetDevice;
    const isDeviceMatch = !detected || normDetected === normTarget;

    // Build device detection info section — only for games with FX data
    let deviceInfoHtml = '';
    if (slot.fxEnabled && detected) {
      // Mini/FX-C share the same CS pin — show combined label
      const detectedLabel = normDetected === DEVICE_TYPE.ARDUBOY_MINI
        ? 'FX-C / Mini' : (DEVICE_LABELS[detected] || detected);
      if (isDeviceMatch) {
        deviceInfoHtml = `
          <div class="cart-device-info cart-device-match">
            <span class="cart-device-icon">&#x2714;</span>
            Compiled for <strong>${this._esc(detectedLabel)}</strong>
          </div>`;
      } else {
        deviceInfoHtml = `
          <div class="cart-device-info cart-device-mismatch">
            <span class="cart-device-icon">&#x26A0;&#xFE0F;</span>
            Compiled for <strong>${this._esc(detectedLabel)}</strong> &mdash; cart targets <strong>${this._esc(DEVICE_LABELS[this.targetDevice] || this.targetDevice)}</strong>
          </div>`;
      }
    }

    // Build multi-binary selector if source .arduboy had multiple binaries
    let binaryVariantsHtml = '';
    if (slot.arduboyBinaries && slot.arduboyBinaries.length > 1) {
      const buttons = slot.arduboyBinaries.map((bin, idx) => {
        const label = DEVICE_LABELS[bin.device] || bin.device || `Binary ${idx + 1}`;
        const isActive = idx === slot.activeBinaryIndex;
        const hasFx = (bin.dataRaw && bin.dataRaw.length > 0) || (bin.saveRaw && bin.saveRaw.length > 0);
        const fxTag = hasFx ? ' <span class="slot-badge slot-badge-fx" style="font-size:8px">FX</span>' : '';
        return `<button class="btn btn-sm ${isActive ? 'btn-primary' : 'btn-outline'} btn-binary-variant" data-bin-index="${idx}" ${isActive ? 'disabled' : ''}>${this._esc(label)}${fxTag}</button>`;
      }).join('');
      binaryVariantsHtml = `
        <div class="cart-binary-variants">
          <span class="cart-binary-label">Source Binaries</span>
          <div class="cart-binary-variant-buttons">${buttons}</div>
        </div>`;
    }
    ...
    panel.innerHTML = `
      ...
      <div class="cart-detail-binaries">
        ...
        <div class="cart-binary-row">
          ...
          <button class="btn btn-xs btn-secondary" id="btn-detail-program">Set .hex</button>
        ...
        <div class="cart-binary-row">
          ...
          <button class="btn btn-xs btn-secondary" id="btn-detail-data">Set .bin</button>
          <input type="file" id="detail-data-file" accept=".bin" class="file-input">
          <button class="btn btn-xs btn-outline btn-danger" id="btn-detail-clear-data" ${dataSize === 0 ? 'disabled' : ''}>Clear</button>
        </div>

        <div class="cart-binary-row">
          <span class="cart-binary-label">FX Save</span>
          <span class="cart-binary-size">${this._formatBytes(saveSize)}</span>
          <button class="btn btn-xs btn-secondary" id="btn-detail-save">Set .bin</button>
          <input type="file" id="detail-save-file" accept=".bin" class="file-input">
          <button class="btn btn-xs btn-outline btn-danger" id="btn-detail-clear-save" ${saveSize === 0 ? 'disabled' : ''}>Clear</button>
        </div>
        <div class="cart-binary-row cart-binary-row-indent">
          <button class="btn btn-xs btn-outline" id="btn-detail-add-save" ${saveSize > 0 ? 'disabled' : ''} title="Add 4KB save area">+4K Save</button>
        </div>
      </div>`;
  ...
  _bindDetailEvents(panel, slot) {
    ...
    const editImageBtn = panel.querySelector('#btn-detail-edit-image');
    const revertImageBtn = panel.querySelector('#btn-detail-revert-image');
    // Show revert button if image has been edited
    if (revertImageBtn && slot._originalImageRaw) {
    ...
    editImageBtn?.addEventListener('click', () => {
      ...
      let imgData;
      if (hasImage) {
        imgData = screenToImageData(slot.imageRaw);
      } else {
        // Create blank 128x64 image (black)
      ...
      if (!slot._originalImageRaw) {
        slot._originalImageRaw = new Uint8Array(slot.imageRaw);
      }
      window.__pixelEditor.open(imgData, {
        filename: slot.meta.title || 'cart-image',
        threshold: 128,
        onSave: (editedImageData) => {
          slot.imageRaw = imageDataToScreen(editedImageData);
          this._markDirty();
          this.render();
        },
      });
    });
    // Double-click canvas to edit
    ...
    revertImageBtn?.addEventListener('click', async () => {
      if (!slot._originalImageRaw) return;
      if (this._confirmDelete && !await showConfirm('Revert image to original?')) return;
      slot.imageRaw = new Uint8Array(slot._originalImageRaw);
      slot._originalImageRaw = null;
      this._markDirty();
      this.render();
    });
    ...
    // Binary variant selector buttons (from multi-binary .arduboy imports)
    panel.querySelectorAll('.btn-binary-variant').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const idx = parseInt(btn.dataset.binIndex, 10);
        await this._switchSlotBinary(slot, idx);
      });
    });
  }
  ...
  /**
   * Infer the cart-level target device from slot binaries.
   * Picks the most common detected device type among game slots.
   */
  _inferTargetDevice() {
    const counts = {};
    for (const slot of this.slots) {
      if (slot.isCategory || !slot.fxEnabled) continue;
      const dev = slot.detectedDevice;
      if (dev) counts[dev] = (counts[dev] || 0) + 1;
    }
    let best = DEVICE_TYPE.ARDUBOY_FX;
    let bestCount = 0;
    for (const [dev, count] of Object.entries(counts)) {
      if (count > bestCount) { best = dev; bestCount = count; }
    }
    this.targetDevice = best;

    // Sync the toolbar dropdown if it exists
    const sel = document.getElementById('cart-target-device');
    if (sel) sel.value = this.targetDevice;
  }

  /**
   * Apply SSD1306/SSD1309 display patch to compiled cart binary based on displayType setting.
   * If SSD1309 is selected, patches 1306→1309. If SSD1306 is selected, patches 1309→1306.
   * @param {Uint8Array} binary - Compiled cart binary (modified in-place)
   */
  _applyDisplayPatch(binary) {
    if (this.displayType === 'SSD1309') {
      const result = patchSSD1309(binary);
      if (result.success) showToast(result.message, 'info');
    } else {
      const result = unpatchSSD1309(binary);
      if (result.success) showToast(result.message, 'info');
    }
  }

  /**
   * Patch mismatched slots' CS pin instructions to match the cart target device.
   * Modifies each slot's programRaw in-place and invalidates caches.
   *
   * ⚠️ Experimental: 2-byte pattern swap that may rarely produce false positives.
   *
   * @param {Array<{index: number, title: string, detected: string}>} mismatches
   */
  _patchMismatchedCSPins(mismatches) {
    let totalPatched = 0;
    let slotsPatched = 0;

    for (const m of mismatches) {
      const slot = this.slots[m.index];
      if (!slot || slot.programRaw.length === 0) continue;

      const result = patchCSPin(slot.programRaw, m.detected, this.targetDevice);
      if (result.success) {
        slot.invalidateDetectedDevice();
    ...
    if (slotsPatched > 0) {
      showToast(`Experimental: Patched CS pin in ${slotsPatched} game(s) (${totalPatched} instruction(s))`, 'info');
      this.dirty = true;
      this.render();
    } else {
  ...
  /**
   * Get list of game slots whose detected device doesn't match the cart target.
   * @returns {Array<{index: number, title: string, detected: string, expected: string}>}
   */
  _getDeviceMismatches() {
    // FX-C and Mini share the same CS pin — treat as compatible
    const normalize = d => d === DEVICE_TYPE.ARDUBOY_FX_C ? DEVICE_TYPE.ARDUBOY_MINI : d;
    const normTarget = normalize(this.targetDevice);
    const mismatches = [];
    for (let i = 0; i < this.slots.length; i++) {
      const slot = this.slots[i];
      if (slot.isCategory || !slot.fxEnabled) continue;
      const detected = slot.detectedDevice;
      if (detected && normalize(detected) !== normTarget) {
        mismatches.push({
          index: i,
          title: slot.meta.title || `Slot ${i + 1}`,
          detected,
          expected: this.targetDevice,
        });
      }
    }
    return mismatches;
  ...
  /**
   * Switch a slot's active binary from its arduboyBinaries array.
   * @param {FxParsedSlot} slot
   * @param {number} binaryIndex - Index into slot.arduboyBinaries
   */
  async _switchSlotBinary(slot, binaryIndex) {
    if (!slot.arduboyBinaries || binaryIndex < 0 || binaryIndex >= slot.arduboyBinaries.length) return;
    if (binaryIndex === slot.activeBinaryIndex) return;

    const bin = slot.arduboyBinaries[binaryIndex];
    ...
    slot.programRaw = programRaw;
    slot.dataRaw = bin.dataRaw || new Uint8Array(0);
    slot.saveRaw = bin.saveRaw || new Uint8Array(0);
    slot.activeBinaryIndex = binaryIndex;
    slot.invalidateDetectedDevice();
    slot._originalImageRaw = null;
  ...
  async _slotFromArduboyFile(file) {
    ...
    return this._slotFromPackage(pkg);
  }

  /**
   * Create an FxParsedSlot from a pre-parsed ArduboyPackage.
   * @param {ArduboyPackage} pkg
   * @returns {Promise<FxParsedSlot>}
   */
  async _slotFromPackage(pkg) {
    if (!pkg.binaries || pkg.binaries.length === 0) {
      throw new Error('No binaries found in .arduboy file');
    }

    // Auto-select binary matching cart target device; fallback to FX/Mini, then first
    const bin = pkg.binaries.find((b) => b.device === this.targetDevice)
      || pkg.binaries.find((b) => b.device === 'ArduboyFX' || b.device === 'ArduboyMini')
      || pkg.binaries[0];

    const activeBinaryIndex = pkg.binaries.indexOf(bin);
    ...
    if (bin.imageRaw && bin.imageRaw.length >= FX_TITLE_SIZE) {
      imageRaw = new Uint8Array(bin.imageRaw);
    } else if (bin.cartImage) {
      try {
        const canvas = new OffscreenCanvas(SCREEN_WIDTH, SCREEN_HEIGHT);
        const ctx = canvas.getContext('2d');
        ctx.drawImage(bin.cartImage, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    // Preserve all binaries from the .arduboy package for multi-binary switching
    if (pkg.binaries.length > 1) {
      slot.arduboyBinaries = pkg.binaries;
      slot.activeBinaryIndex = activeBinaryIndex;
    }
  ...
  _updateSlotListItem(index, slot) {
    ...
    if (slot.isCategory) {
      ...
      if (titleEl) {
        ...
        const ssd1309Badge = slot.hasSSD1309Patch ? ' <span class="slot-badge slot-badge-1309">1309</span>' : '';
        titleEl.innerHTML = `${this._esc(slot.meta.title || 'Untitled')}${deviceBadge}${ssd1309Badge}`;
      }
    }
  }
  ...
  /** True if the cart has content beyond a fresh "New" cart (2 empty category slots). */
  _hasNonTrivialContent() {
    if (this.slots.length === 0) return false;
    if (this.slots.length > 2) return true;
    return this.slots.some((s) => s.programRaw.length > 0 || s.dataRaw.length > 0 || s.saveRaw.length > 0);
  }
  ...
  /** Schedule a debounced save to localStorage (used by _markDirty for text input). */
  _scheduleSave() {
    clearTimeout(this._saveTimer);
    this._saveTimer = setTimeout(() => this._saveToStorage(), STORAGE_SAVE_DELAY);
  }

  /** Serialise the current cart state to localStorage. */
  _saveToStorage() {
    clearTimeout(this._saveTimer);
    try {
      if (this.slots.length === 0) {
      ...
      const data = {
        slots: this.slots.map((s) => ({
          category: s.category,
          imageRaw: _u8ToBase64(s.imageRaw),
          programRaw: _u8ToBase64(s.programRaw),
          dataRaw: _u8ToBase64(s.dataRaw),
          saveRaw: _u8ToBase64(s.saveRaw),
          meta: { ...s.meta },
        })),
  ...
  /** Restore cart state from localStorage on startup. */
  _restoreFromStorage() {
    try {
      ...
      this.slots = data.slots.map((s) => new FxParsedSlot({
        category: s.category,
        imageRaw: _base64ToU8(s.imageRaw),
        programRaw: _base64ToU8(s.programRaw),
        dataRaw: _base64ToU8(s.dataRaw),
        saveRaw: _base64ToU8(s.saveRaw),
        meta: s.meta || { title: '', version: '', developer: '', info: '' },
      }));
      this.selectedIndex = typeof data.selectedIndex === 'number' ? data.selectedIndex : -1;
      this.dirty = !!data.dirty;
      this.filename = data.filename || '';

      // Restore device target and display type
      if (data.targetDevice) {
        this.targetDevice = data.targetDevice;
        const targetSel = document.getElementById('cart-target-device');
        if (targetSel) targetSel.value = data.targetDevice;
      }
      if (data.displayType) {
        this.displayType = data.displayType;
        const displaySel = document.getElementById('cart-display-type');
        if (displaySel) displaySel.value = data.displayType;
      }
