...
import { parseDimensionsFromFilename, loadImageFromBytes } from '../core/fxdata/fxdataImageEncoder.js';
...
// Maximum line width for array values before wrapping
const ARRAY_LINE_WIDTH = 80;
...
/**
 * Wrap a comma-separated value string into multiple lines at a reasonable width.
 * @param {string} csv - e.g. "1, 2, 3, 4, 5, 6, ..."
 * @returns {string} wrapped with newline + tab
 */
function wrapArrayValues(csv) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const candidate = currentLine ? `${currentLine}${item}${sep}` : `${item}${sep}`;
    if (currentLine && candidate.length > ARRAY_LINE_WIDTH) {
  ...
  if (currentLine.trimEnd()) wrappedLines.push(currentLine.trimEnd());
  return wrappedLines.join('\n\t');
}

export function entriesToSource(entries) {
  const lines = [];
  // Start with the header comment (trimEnd to avoid extra blank lines from join)
  lines.push(FXDATA_HEADER.trimEnd());
...
export function sourceToEntries(source) {
  ...
  // Strip block comments (stateful scan to handle nested /* */ text correctly)
  let text = '';
  for (let i = 0; i < source.length; i++) {
    if (source[i] === '/' && source[i + 1] === '*') {
      // Entered block comment — scan for the real closing */
      i += 2;
  ...
  for (const rawLine of joinedLines.split('\n')) {
    // Extract inline comment (skip // inside quoted strings)
    let line = rawLine;
    let comment = '';
    let inQ = false;
    let qChar = '';
...
export class FxDataEditor {
  constructor() {
    ...
    /** @type {Set<string>} Tracks image paths that have been edited in the pixel editor */
    this._editedImages = new Set();

    /** @type {Map<string, Uint8Array>} Original image bytes before any pixel editor edits */
    this._originalImageBytes = new Map();
  ...
  _grabRefs() {
    ...
    this._btnNewImage = $('#fxdata-btn-new-image');
  ...
  _bindEvents() {
    ...
    this._btnPush?.addEventListener('click', () => this._pushToCode());
  ...
  /** Update entry values that reference old file paths after move/rename */
  _updateEntryPaths(pathMap) {
    let changed = false;
    for (const entry of this._entries) {
      if (entry.type !== 'image_t' && entry.type !== 'raw_t') continue;
      const unquoted = entry.value.replace(/^["']|["']$/g, '');
      if (pathMap.has(unquoted)) {
        entry.value = `"${pathMap.get(unquoted)}"`;
        changed = true;
      }
    }
    if (changed) {
  ...
  _updateToolbarButtons() {
    ...
    if (this._btnExport) this._btnExport.disabled = !hasEntries;
    if (this._btnBuild) this._btnBuild.disabled = !hasEntries;
    if (this._btnPush) this._btnPush.disabled = !hasEntries;
  }
  ...
  async _createNewImage() {
    ...
    if (this._project.hasFile(path) && !this._overwriteByDefault) {
      const ok = await showConfirm(`"${path}" already exists. Overwrite?`, { danger: true });
      if (!ok) return;
    }
    this._project.addFile(path, result.pngBytes);
    this._renderAssetTree();
    this._renderEntriesPanel();
    this._saveToStorage();
    showToast(`Created ${path} (${result.width}\u00D7${result.height})`, 'success');
  }
    ...
    const commit = () => {
      ...
      if (newFolder && newFolder !== oldFolder) {
        ...
        for (const p of toMove) {
          ...
          this._project.addFile(newFilePath, fileData.data);
          this._project.removeFile(p);
          renamedMap.set(p, newFilePath);
        }
        ...
        this._updateEntryPaths(renamedMap);
        this._saveToStorage();
    ...
    const commit = () => {
      ...
      if (newName && newName !== oldName) {
        ...
        if (this._project.hasFile(newPath)) {
          ...
          this._updateEntryPaths(new Map([[oldPath, newPath]]));
          this._saveToStorage();
  ...
  _moveFolder(oldPath, targetFolder) {
    ...
    for (const p of toMove) {
      const suffix = p.slice(oldPath.length);
      movedMap.set(p, newPath + suffix);
    }
    this._updateEntryPaths(movedMap);
    this._renderAssetTree();
    this._renderEntriesPanel();
    this._saveToStorage();
    const dest = targetFolder || '(root)';
  ...
  async _pushToCode() {
    ...
    // Sync source so entries are up-to-date
    if (this._sourceDirty) this._syncSourceToEntries();

    // ── Gather files to push ──────────────────────────────────────────
    const filesToPush = [];  // { filename, data (Uint8Array | string), isBinary }

    // 1. Asset files from the project VFS (images, raw bins in folders)
    const assetFiles = this._project.listFiles();
    for (const path of assetFiles) {
      ...
      filesToPush.push({ filename: path, data, isBinary: true });
    }

    // 2. fxdata.txt (always push current entries as source)
    const fxdataSource = entriesToSource(this._entries);
    if (fxdataSource.trim()) {
      filesToPush.push({
        filename: 'fxdata.txt',
        data: fxdataSource,
        isBinary: false,
    ...
    // 3. Build outputs (if built)
    if (this._lastBuild?.success) {
      filesToPush.push({
        filename: 'fxdata.h',
        data: this._lastBuild.header,
        isBinary: false,
      });
      filesToPush.push({
        filename: 'fxdata.bin',
        data: this._lastBuild.devBin,
        isBinary: true,
      });
      filesToPush.push({
        filename: 'fxdata-data.bin',
        data: this._lastBuild.dataBin,
        isBinary: true,
      });
      if (this._lastBuild.saveBin) {
        filesToPush.push({
          filename: 'fxdata-save.bin',
          data: this._lastBuild.saveBin,
          isBinary: true,
        });
      }
    }
    ...
    if (filesToPush.length === 0) {
      showToast('No files to push', 'warning');
      return;
    }
    ...
    for (const f of filesToPush) {
      const existing = api.findTabByFilename(f.filename);
      if (existing) conflicts.push(f.filename);
    }

    if (conflicts.length > 0) {
      const listHtml = conflicts.length <= 8
        ? conflicts.map((n) => `<li>${n}</li>`).join('')
        : conflicts.slice(0, 7).map((n) => `<li>${n}</li>`).join('')
          + `<li>… and ${conflicts.length - 7} more</li>`;
      const ok = await showConfirm(
        `The following ${conflicts.length} file(s) already exist in the code editor and will be replaced:\n\n`
        + conflicts.join(', '),
        { title: 'Replace Files', okLabel: 'Replace', cancelLabel: 'Cancel' },
      );
      if (!ok) return;
    }
    ...
    for (const f of filesToPush) {
      if (f.isBinary) {
        // Binary file — use createBinaryTab (handles updates if existing)
        const binaryData = f.data instanceof Uint8Array ? f.data : new TextEncoder().encode(f.data);
        // Close any existing text tab with the same name first
        const existingText = api.findTabByFilename(f.filename);
        if (existingText && !existingText.isBinary) {
          api.closeTab(existingText.id);
        }
        api.createBinaryTab(f.filename, binaryData.length, binaryData);
        pushedBinary++;
      } else {
        // Text file — use createTab or update existing model
        const content = typeof f.data === 'string' ? f.data : new TextDecoder().decode(f.data);
        const existing = api.findTabByFilename(f.filename);
        if (existing && existing.model) {
          existing.model.setValue(content);
        } else {
          // Close any existing binary tab with the same name
          if (existing && existing.isBinary) {
            api.closeTab(existing.id);
          }
          api.createTab(f.filename, content);
        }
        pushedText++;
      }
    }
    ...
    // Try to switch to fxdata.txt in the editor
    const fxTab = api.findTabByFilename('fxdata.txt');
    if (fxTab) api.switchToTab(fxTab.id);

    const parts = [];
    if (pushedText > 0) parts.push(`${pushedText} text`);
    if (pushedBinary > 0) parts.push(`${pushedBinary} binary`);
    showToast(`Pushed ${parts.join(' + ')} file(s) to Code editor`, 'success');
  }
  ...
  _validateEntries() {
    ...
    // Account for the header comment lines + blank separator lines between entries.
    const headerLines = FXDATA_HEADER.trimEnd().split('\n').length;
    ...
    for (const d of diagnostics) {
      if (!d.line) {
        ...
        // headerLines + 1 blank separator before first entry, then +2 per subsequent entry
        // (1 blank line between entries + the entry line)
        d.line = idx >= 0 ? headerLines + 2 + idx * 2 : 0;
      }
    }
    return diagnostics;
  }
  ...
  _showAssetPreviewByPath(path, variableName, sourceType) {
    ...
    if (isImage) {
      // Build scale controls row HTML
      const currentScale = this._fxPreviewScale || '1x';
      const scaleValues = ['1x', '2x', '4x', 'fill'];
      const scaleLabels = ['1\u00D7', '2\u00D7', '4\u00D7', 'Fill'];
      html += `<div class="fxdata-preview-controls"><div class="fxdata-scale-btns" role="group" aria-label="Preview scale">`;
      for (let si = 0; si < scaleValues.length; si++) {
        const checked = scaleValues[si] === currentScale ? ' checked' : '';
        html += `<label class="fxdata-scale-btn"><input type="radio" name="fxdata-preview-scale" value="${scaleValues[si]}"${checked}> ${scaleLabels[si]}</label>`;
      }
      html += `</div>`;
      if (this._editedImages?.has(path)) {
        html += `<span class="pe-edited-badge">edited</span>`;
      }
      html += `<button type="button" class="btn btn-secondary btn-sm fxdata-edit-btn" data-path="${escapeHtml(path)}" title="Open pixel editor">Edit</button>`;
      if (this._editedImages?.has(path)) {
        html += `<button type="button" class="btn btn-danger btn-sm fxdata-revert-btn" data-path="${escapeHtml(path)}" title="Revert to original image">Revert</button>`;
      }
      html += `</div>`;
      ...
      // Bind scale button events after inserting HTML
      this._assetPreview.querySelectorAll('input[name="fxdata-preview-scale"]').forEach(r => {
        r.addEventListener('change', () => this._applyFxPreviewScale());
      });

      // Bind edit button
      const editBtn = this._assetPreview.querySelector('.fxdata-edit-btn');
      if (editBtn) {
        editBtn.addEventListener('click', () => this._openPixelEditor(path, data));
      }

      // Double-click preview image to open pixel editor
      const previewImg = this._assetPreview.querySelector('.fxdata-preview-img');
      if (previewImg) {
        previewImg.title = 'Double-click to open pixel editor';
        previewImg.addEventListener('dblclick', () => this._openPixelEditor(path, data));
      }

      // Bind revert button
      const revertBtn = this._assetPreview.querySelector('.fxdata-revert-btn');
      if (revertBtn) {
        revertBtn.addEventListener('click', () => this._revertImage(path));
      }
  ...
  /**
   * Open the pixel editor modal for an FX data image asset.
   */
  async _openPixelEditor(path, data) {
    if (!window.__pixelEditor) return;
    let imageData;
    try {
      imageData = await loadImageFromBytes(data);
    } catch {
      showToast('Failed to load image for editing', 'error');
      return;
    }

    // Stash original bytes before first edit
    if (!this._originalImageBytes.has(path)) {
      this._originalImageBytes.set(path, new Uint8Array(data));
    }
    ...
    window.__pixelEditor.open(imageData, {
      filename: path,
      threshold,
      onSave: async (editedImageData, meta) => {
        // Convert edited ImageData back to PNG bytes
        const canvas = new OffscreenCanvas(editedImageData.width, editedImageData.height);
        ...
        // Update project VFS
        this._project.addFile(path, bytes);

        // Mark as edited
        this._editedImages.add(path);

        // Update threshold
        if (this._thresholdSlider) this._thresholdSlider.value = meta.threshold;

        // Re-render preview
        this._showAssetPreviewByPath(path, this._currentPreviewVar, this._currentPreviewSourceType);
      ...
      onThresholdChange: (val) => {
        if (this._thresholdSlider) {
          this._thresholdSlider.value = val;
          const valLabel = this._thresholdSlider.parentElement?.querySelector('.fxdata-threshold-value');
  ...
  /**
   * Revert an image asset to its original imported bytes.
   */
  async _revertImage(path) {
    const original = this._originalImageBytes.get(path);
    if (!original) return;
    ...
    if (!ok) return;
    this._project.addFile(path, new Uint8Array(original));
    this._editedImages.delete(path);
    this._originalImageBytes.delete(path);
    this._showAssetPreviewByPath(path, this._currentPreviewVar, this._currentPreviewSourceType);
    this._saveToStorage();
  ...
  _applyFxPreviewScale() {
    const canvas = this._assetPreview?.querySelector('.fxdata-preview-img');
    if (!canvas) return;
    const checked = this._assetPreview.querySelector('input[name="fxdata-preview-scale"]:checked');
    const scale = checked ? checked.value : '1x';
    this._fxPreviewScale = scale;
    ...
    canvas.classList.remove('fxdata-fill-view');
    ...
    if (scale === '2x') {
      ...
      canvas.classList.add('fxdata-fill-view');
    }
  ...
  _saveToStorage() {
    try {
      ...
      if (this._editedImages.size > 0) {
        data.editedImages = [...this._editedImages];
      }
      if (this._originalImageBytes.size > 0) {
        const origEntries = [];
        for (const [path, bytes] of this._originalImageBytes) {
          let bin = '';
          for (let i = 0; i < bytes.length; i++) bin += String.fromCharCode(bytes[i]);
          origEntries.push([path, btoa(bin)]);
        }
        data.originalImageBytes = origEntries;
      }
  ...
  _restoreFromStorage() {
    try {
      ...
      if (Array.isArray(data.editedImages)) {
        this._editedImages = new Set(data.editedImages);
      }
      if (Array.isArray(data.originalImageBytes)) {
        this._originalImageBytes = new Map();
        for (const [path, b64] of data.originalImageBytes) {
          const raw = atob(b64);
          const bytes = new Uint8Array(raw.length);
          for (let i = 0; i < raw.length; i++) bytes[i] = raw.charCodeAt(i);
          this._originalImageBytes.set(path, bytes);
        }
      }
      this._updateToolbarButtons();
    } catch {
